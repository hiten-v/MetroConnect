  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex items-center gap-2 md:gap-3 mb-3 flex-wrap">
        <div className="w-3 h-3 md:w-3.5 md:h-3.5 rounded-full flex-shrink-0"
                  ...
                  >
                    {(prov, snap) => (
                      <div ref={prov.innerRef} {...prov.draggableProps}
                        style={prov.draggableProps.style}
                        className={`flex items-center gap-2 px-2.5 md:px-3 py-2 rounded-lg border
                                    text-sm transition-colors
                          ${snap.isDragging
                            ? 'bg-surface2 border-accent shadow-lg'
                            : 'bg-white/[0.02] border-metro'}`}
                      >
                        <span {...prov.dragHandleProps}
                          className="text-muted cursor-grab text-sm select-none px-0.5">
                          ⋮⋮
                        </span>

                        <span className="w-5 h-5 rounded-full text-[10px] flex items-center
                                         justify-center font-bold flex-shrink-0"
                          style={{ background: line.color + '25', color: line.color }}>
                          {idx + 1}
                        </span>

                        <span className="flex-1 text-[#e8eaf6] text-xs truncate">{name}</span>

                        {isXchg && (
                          <span className="text-[10px] bg-accent/20 text-accent px-1.5 py-0.5
